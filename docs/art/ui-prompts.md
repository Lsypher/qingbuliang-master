...
**导入设置固定（交付前对一眼）**：标题艺术字与奖杯图标这两张**透明底裁切件**保持
「裁剪 = 关闭（保留原始画布）」「参与自动图集 = 关」——前者是因为五字被裁会导致定位漂移，
后者是避免将来开启图集时边缘渗色。按钮底图铺满画布、不需要留白裁切，同样不参与图集。
连 `.meta` 一起覆盖最省事；只丢 PNG 的话，新导入的文件会回到默认设置，需要手工再关一次。
