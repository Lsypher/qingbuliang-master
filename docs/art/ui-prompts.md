...
给《清补凉大师》**开始页**出 3 类共 4 张图：标题艺术字 1 张、开始按钮底图 2 张（正常 / 按下）、最高分奖杯图标 1 张。以下提示词可直接复制给美术，或复制进生图工具当做出图依据（中文版适合即梦 / 通义万相 / 文心一格，英文版适合 Midjourney / Stable Diffusion）。

> **素材来源**：三张图按**项目自备（用户交付）**处理，只用授权明确的素材（见 `docs/adr/0001-asset-sourcing-and-licensing.md`）。提示词是可复制的出图依据；若用生图工具产出，请先确认生成物的授权与可用性再交付。

## 通用规格（每张都必须满足）

| 项 | 要求 |
|---|---|
| 画幅 | 一律**横平竖直的正构图**（正投影、左右对称），不要四边加边框、不要透视斜角 |
| 导出倍率 | **2 倍**：设计像素 ×2 导出（每张的导出尺寸见各自规格表）；导出后不要再缩放，由引擎缩 |
| 透明通道 | **必须是 32 位透明底 PNG**（带 alpha）；不要白底 / 黑底，不要 JPG |
| 四周透明留白 | 见每张的规格表逐项写明；留白必须**四边等宽、完全透明**，图不贴画布边 |
...
| 九宫格准备 | 四角装饰必须整块落在**距画布边缘 80px（设计）以内**的角部、且上下方向控制在 **40px** 内；中间区域必须是**可无限拉伸的均匀平纹**（无独立图案、无焦点），横向拉伸时不能出现被拉长的具象东西 |
| 九宫格切边（给程序用） | left / right = 80、top / bottom = 40（设计像素；2 倍图即 160 / 80） |
...
7. **按钮两态可辨**：正常态与按下态放一起能明显看出差别；四角装饰都落在角部（距边 80px 内）、中间是均匀平纹。
